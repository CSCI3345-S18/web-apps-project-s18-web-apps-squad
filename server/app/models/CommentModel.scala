...
object CommentModel {
  ...
  def addComment(c: NewComment, db: Database)(implicit ec: ExecutionContext): Future[Int] = {
    db.run {
      //0 for id upvotes and downvotes
      comments += Comment(0, c.body, c.userID, c.postParentID, c.commentParentID,  c.flag, 0, 0)
    }
  }
}
